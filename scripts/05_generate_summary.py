"""Generate M3 summary markdown from loss CSV."""
import csv

CSV = "/root/autodl-tmp/vla/results/03_loss_data.csv"
OUT = "/root/autodl-tmp/vla/results/03_m3_training_summary.md"

rows = list(csv.DictReader(open(CSV)))
first, last = rows[0], rows[-1]

md = f"""# M3 Results — SmolVLA Fine-tuning on SO-101 PickPlace
...
|---|---|---|---|
| Step 100 (start) | {first['loss']} | {first['grad_norm']} | {first['lr']} |
| Step 20,000 (final) | **{last['loss']}** | {last['grad_norm']} | {last['lr']} |
| Smoke test (500 steps, M2.5) | 0.196 | 2.74 | - |

## Observations

1. **Strong convergence**: Loss decreased from {first['loss']} → {last['loss']}, a {(1 - float(last['loss'])/float(first['loss']))*100:.0f}% reduction over 20K steps.
...
print(f"Final loss: {last['loss']}")
print(f"Final grad norm: {last['grad_norm']}")
